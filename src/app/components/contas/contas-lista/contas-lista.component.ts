...
export class ContasListaComponent implements OnInit {
  ...
  displayedColumns =  ['Idconta', 'descricao', 'tipo', 'actions'];
 // dataSource = new MatTableDataSource<PeriodicElement>(ELEMENT_DATA);
//  @ViewChild(MatPaginator, {read: true}) paginator: MatPaginator;
dataSource = new MatTableDataSource(ELEMENT_DATA);
@ViewChild(MatPaginator) paginator!: MatPaginator;
@ViewChild(MatSort) sort: MatSort = new MatSort;
  listaContas: ContasModel[] = [];
  constructor(private paginacao: MatPaginator,
              private contasService: ContasService,
              ) {}

  ngOnInit(): void {
    this.contasService.getAll().subscribe((data )=>{ this.listaContas = data });
